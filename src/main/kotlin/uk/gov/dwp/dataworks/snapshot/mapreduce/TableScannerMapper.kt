...
class TableScannerMapper: TableMapper<Text, Text>() {
    ...
    override fun map(id: ImmutableBytesWritable, result: Result, context: Context) {
        processor.process(result)?.let { decryptedRecord ->
            val row = result.row
            ...
            val dbObject =
                Text().apply { set(StringBuilder(decryptedRecord.dbObject.toString()).append('\n').toString()) }
            context.write(Text().apply { set(key) }, dbObject)
        }
    }

    private val processor: Processor<Result, DecryptedRecord> by lazy {
